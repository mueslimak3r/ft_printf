...
size_t			justify2(t_buffer *b, t_flags *f, char t, int s)
{
	...
	if ((f->zero == false) && f->max_size > s)
		...
		ret += (f->max_size > s - f->plus) ? ft_savechar(b, '0', (f->max_size - s - f->plus)) : ft_savechar(b, '0', (f->min_len - s - f->plus));
	return (ret);
}
...
size_t			justify_d(t_buffer *b, t_flags *f, char t, int s, int p)
{
	...
	if (p == 0)
	{
		if (f->min_len > s && f->min_len > f->max_size && f->max_size > s && f->minus == false)
			ret += ft_savechar(b, ' ', (f->min_len - f->max_size - f->plus - f->isneg));
	...
	else if (p == 1)
	{
		if (f->min_len > s && f->min_len > f->max_size && f->max_size > 0 && f->minus == true)
			ret += ft_savechar(b, ' ', (f->min_len - f->max_size - f->plus - f->isneg));
		else if (f->min_len > s && (f->min_len > f->max_size) && f->minus == true)
...
size_t			parse_d(t_buffer *b, t_flags *f)
{
	...
	size = ((f->l_size && f->max_size == 0) && f->inbuf->s == 0) ? 0 :(int)ft_strlen(str);
	f->plus = (f->inbuf->s >= 0 && f->plus) ? 1 : 0;
	ret += justify_d(b, f, 'd', size, 0);
	ret += ((f->l_size && f->max_size == 0) && f->inbuf->s == 0) ? 0 : ft_savestr(b, str + f->isneg, (int)ft_strlen(str + f->isneg));
